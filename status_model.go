...
func (d *DynamicRole) UnmarshalJSON(data []byte) error {
	...
	case "commit_proxy":
		d.Value = new(DynamicCommitProxyRole)
	}
	return json.Unmarshal(data, d.Value)

}

// DynamicCommitProxyRole is added manually
type DynamicCommitProxyRole struct {
	Role               string `json:"role"`
	ID                 string `json:"id"`
	BatchingWindowSize struct {
		Count  float64 `json:"count"`
		...
		P99_9  float64 `json:"p99.9"`
	} `json:"commit_batching_window_size"`
	CommitLatencyStatistics struct {
		Count  float64 `json:"count"`
		Max    float64 `json:"max"`
		Mean   float64 `json:"mean"`
		Median float64 `json:"median"`
		...
		P99    float64 `json:"p99"`
		P99_9  float64 `json:"p99.9"`
	} `json:"commit_latency_statistics"`
}
